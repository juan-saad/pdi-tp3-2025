...
cap = cv2.VideoCapture(
    "tirada_1.mp4"
)  # Abre el archivo de video especificado ('tirada_1.mp4') para su lectura.
width = int(
...
out = cv2.VideoWriter(
    "Video-Output.mp4", cv2.VideoWriter_fourcc(*"mp4v"), fps, (width, height)
)
...
while cap.isOpened():  # Verifica si el video se abrió correctamente.
    ...
    if ret == True:
        ...
        frame_show = cv2.resize(
            frame, dsize=(int(width / 3), int(height / 3))
        ...
        if cv2.waitKey(25) & 0xFF == ord(
            "q"
        ):  # Espera 25 milisegundos a que se presione una tecla. Si se presiona 'q' se rompe el bucle y se cierra la ventana.
            break
    else:
        break
